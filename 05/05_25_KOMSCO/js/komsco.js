    ...
    var komsco = {
        ...
        headerFn:function(){
            ...
            $a.on({
                click:function(e){
                    ...
                    var url = $(this).attr('href');
                        location.href = 'http://localhost/05_25_KOMSCO' + url;
                }
            });


            function cookieSetFn(){
                var txt = '';
                var today = new Date();
                ...
                // 2. 쿠키 날짜지정(만료일)
                // 하루동안 열리지 않음 (1일간의 쿠키 만료일 기간설정)
                today.setDate(today.getDate()+1);
    
    
                // 3. 쿠키설정
                txt += 'popup20210525 = no;';
                // txt += 'expires=' + 오늘날짜.toGMTString();
                // txt += 'expires=' + 오늘날짜.toUTCString();
                txt += 'expires=' + today.toUTCString() + ';';  // 표준시
    
                document.cookie = txt;  // 최종 쿠키설정
            ...
            $('.close-btn').on({
                click:function(e){
                    e.preventDefault();
                    if(document.cookieForm.check.checked === true){
                        cookieSetFn();
                    }
                }
            });
